...
async function syncOneUser(connection: { owner_id: string; refresh_token_secret_id: string; calendar_id: string }) {
  ...
  if (rows.length) {
    const { error } = await supabase.from('calendar_events').upsert(rows, { onConflict: 'owner_id,google_event_id' });
    if (error) throw error;
  }

  let cleanupQuery = supabase.from('calendar_events').update({ deleted_at: new Date().toISOString() })
    .eq('owner_id', ownerId).eq('source_module', 'google').not('google_event_id', 'is', null)
